...
$query2 = "select u_id,u_name from hospital.users where role_id=2";
...
$query = "select issue_id,department_id,date,u_id from inventory.issues where date>='$newDate'";
...
while($row = mysqli_fetch_array($res,MYSQLI_ASSOC)){
  ...
  $query3 = "select serial_number from inventory.issue_details where issue_id='$issue_id'";
  $res3 = mysqli_query($conn,$query3);
  if(!$res3){
    die("Server Error");
  }
  if(mysqli_num_rows($res3)>0){
  $printHtml = $printHtml."<tr> <td>".$row["date"]."</td><td>".$departments[$row["department_id"]]."</td><td>".$users[$row["u_id"]]."</td><td><button class='btn btn-info' value=".$row["issue_id"]." onclick='viewDetails(this);'>Details</button></td></tr>";
  }
}
  ...
  <table id="myTable" class="table table-striped">  
    <thead>  
      <tr>  
        ...
        <th>Department<img alt="sort" src="../images/sort2.png" style="width:25px;height:25px;" ></th>   
